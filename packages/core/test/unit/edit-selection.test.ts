...
describe("選択の持ち方", () => {
  it("掴んだ側と動く側で持ち、range は前後に揃える", () => {
    ...
    expect(selection(state)).toEqual({ anchor: 5, focus: 2 });
  ...
  it("本文の外は端に丸める", () => {
    ...
    expect(selection(state)).toEqual({ anchor: 0, focus: 3 });
  });

  it("元の state は書き換わらない", () => {
    const before = newEditState("あいう");
    setSelection(before, 1, 3);
    expect(selection(before)).toEqual({ anchor: 0, focus: 0 });
  });
...
describe("キャレットを動かす", () => {
  it("extend でないと掴んだ側も動く", () => {
    ...
    expect(selection(extended)).toEqual({ anchor: 1, focus: 3 });

    const moved = moveCaret(extended, { offset: 4, preferEnd: false }, false).state;
    expect(selection(moved)).toEqual({ anchor: 4, focus: 4 });
  });
...
describe("まとまりで選ぶ", () => {
  it("語を選ぶ", () => {
    ...
    expect(selection(result.state)).toEqual({ anchor: 0, focus: 2 });
  });
  ...
  it("つまみを掴むと、掴んだ側が focus になる", () => {
    ...
    expect(selection(grabHandle(selected, "start").state)).toEqual({ anchor: 4, focus: 1 });
    expect(selection(grabHandle(selected, "end").state)).toEqual({ anchor: 1, focus: 4 });
  });
});
