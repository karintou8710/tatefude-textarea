...
インライン方向が `↑` `↓`、ブロック方向が `←` `→` になる (横書きはその逆)。
これは Linux / Windows の `<textarea>` と同じ割り当て。
macOS だけは矢印が OS のキーバインドから来るため、縦書きでも `←` `→` がインライン方向のままで、
**そこだけ合わせていない。**
...
**修飾キーはその OS のネイティブに合わせる**。語・段落へ動くのが macOS では `⌥`、
それ以外では `Ctrl`。端まで飛ぶのは macOS が `⌘` + 矢印、それ以外は `Home` / `End`。
...
| 単語ぶん動く | `⌥` + `↑` `↓` | `Ctrl` + `↑` `↓` |
| 段落の頭 / 末へ | `⌥` + `←` `→` | `Ctrl` + `←` `→` |
| 行頭 / 行末へ | `⌘` + `↑` `↓`、`Home` `End` | `Home` `End` |
| 文頭 / 文末へ | `⌘` + `←` `→` | `Ctrl` + `Home` `End` |
| 選択を伸ばす | `Shift` + 上記 | ← 同じ |
| 全選択 / 取り消し / やり直し | `⌘A` / `⌘Z` / `⇧⌘Z` | `Ctrl` に読み替え |
| 単語 / 段落を選ぶ | ダブルクリック / トリプルクリック | ← 同じ |

ホイールとタッチのパンでブロック方向へスクロールする (縦書きなので、下に回すと左へ読み進む)。
