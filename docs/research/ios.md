...
| 症状 | 原因 | 直し方 |
| --- | --- | --- |
| タップした列と別の列にキャレットが行く | focus を先に入れると、その時点の古いキャレットを見せるスクロールが走る | 測る → 置く → focus の順にする |
| キーボードが開いた瞬間に列が飛ぶ | スクロールできる上限 (spacer) を直す前にスクロールしていた | `syncGeometry` で 寸法 → レイアウト → 上限 を固定 |
